...
https://cs450.class.uic.edu/assignments/homework-6-a-raw-socket-ping-tool/
Textbook- Computer Networking A Top Down Approacj
https://stackoverflow.com/questions/10008144/creating-an-icmp-traceroute-in-python
https://pypi.org/project/icmplib/
https://www.gonwan.com/2013/04/21/ping-using-a-raw-socket/
https://www.winsocketdotnetworkprogramming.com/winsock2programming/winsock2advancedrawsocket11.html
...
class IcmpHelperLibrary:
    ...
    class IcmpPacket_EchoReply:
        ...
        '''Create getters for the validity of each of the sequence number, identifier, and the raw data.'''
        ...
        def printResultToConsole(self, ttl, timeReceived, addr):
            ...
            '''Identify if the echo response is valid and report the error information details. For example, 
            ...
            if not self.isValidResponse():
                ...
                if not self.getIsValidSequenceNumber():
                    print("Expected Sequence Number:", self.getPacketSequenceNumber(), "  Actual Sequence Number:", self.getIcmpSequenceNumber())
    ...
    def __sendIcmpEchoRequest(self, host):
        ...
        for i in range(4):
            ...
            '''Your program can only detect timeouts in receiving ICMP echo responses. Modify 
            the Pinger program to parse the ICMP response error codes and display the 
            corresponding error results to the user. Examples of ICMP response error codes 
            are 0: Destination Network Unreachable, 1: Destination Host Unreachable. 
            ...
            if (icmpPacket.getIcmpType() != 0) and (icmpPacket.getIcmpType() in icmpErrorCodes): #if an error is found
                print("There was an error. ICMP type: " + icmpPacket.getIcmpType() + ", ICMP code: " + icmpPacket.getIcmpCode() +
                      ": " + icmpErrorCodes[icmpPacket.getIcmpType()][icmpPacket.getIcmpCode()]) #print out the error type and the error code

        '''Modify the code to correspond to the way the standard
         ping program works. You will need to report the minimum, maximum, and average RTTs at
          the end of all pings from the client. In addition, calculate the packet loss rate
           (in percentage). It is recommended to create an output that is easily readable with 
        ...
        print("\nRound Trip Times:")
        print("Minimum:", min(self.round_trip_times) * 1000, "ms;", "Maximum:", max(self.round_trip_times) * 1000, "ms;", "Average: ", average_round_trip_times, "ms")
    ...
    def __sendIcmpTraceRoute(self, host):
        ...
        while icmpType != 0:
            traceroute = IcmpHelperLibrary.IcmpPacket()
            traceroute.setTtl(SequenceNumber)

            packetIdentifier = (os.getpid() & 0xffff) # Get as 16 bit number - Limit based on ICMP header standards
                                                           # Some PIDs are larger than 16 bit
            tracerouteSequenceNumber = SequenceNumber #set the traceroute's sequence number to the sequence number previously established

            traceroute.buildPacket_echoRequest(packetIdentifier, tracerouteSequenceNumber) # Build ICMP for IP payload
            traceroute.setIcmpTarget(host)
            icmpType = traceroute.sendEchoRequest()  # Build the IP

            traceroute.printIcmpPacketHeader_hex() if self.__DEBUG_IcmpHelperLibrary else 0
            traceroute.printIcmpPacket_hex() if self.__DEBUG_IcmpHelperLibrary else 0
...
def main():
    ...
    # icmpHelperPing.sendPing("www.chinadaily.com.cn")
    # icmpHelperPing.sendPing("www.ed.ac.uk")
